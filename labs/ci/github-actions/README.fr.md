...
🔗 [**CI Ansible avec GitHub Actions**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/roles/ci-github-actions/)
...
3. Cacher le `pip install` entre runs avec **`actions/setup-python@83679a892e2d95755f2dac6acb0bfd1e9ac5d548 # v6.1.0`**.
4. Configurer des **permissions minimales** (`permissions: {}`).
...
permissions: {}             # moindre privilège : on accorde par job, au strict besoin
...
- **`matrix.distro × matrix.ansible`** = **9 jobs en parallèle**.
...
- **Dependabot** : auto-PR pour mettre à jour les versions des actions
  (il met à jour le SHA épinglé **et** son commentaire `# vX.Y.Z`, ce qui rend l'épinglage tenable).
