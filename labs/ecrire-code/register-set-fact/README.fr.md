...
crée ou recalcule une variable au runtime : niveau 19 dans la précédence (donc bat
...
**Niveau de précédence** : 19. `set_fact` (et les variables `registered`, même
niveau) bat `vars:` du play (12), `vars_files:` (14), `include_vars` (18) et tous
les group_vars/host_vars. Pratique pour **forcer** une valeur calculée. Seuls les
role params (20), les include params (21) et `--extra-vars` (22) passent au-dessus.
...
- **`set_fact:`** crée une variable au runtime : niveau 19 dans la précédence
  (à ne pas confondre avec `include_vars`, qui est 18, juste en dessous).
...
2. Pourquoi `set_fact:` est-il **niveau 19** (plus prioritaire que `vars:` du play à 12,
   et que `vars_files:` à 14) ? Quel cas d'usage justifie cette précédence ?
