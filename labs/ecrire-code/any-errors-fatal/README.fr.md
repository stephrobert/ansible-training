...
PLAY RECAP : `web1: ok=1 failed=1`, `web2: ok=1 failed=0`. **web2 n'est pas
marqué en échec** : il n'a planté sur aucune tâche. Ce qui prouve l'arrêt, c'est
son compteur `ok` : **1 au lieu de 2**, parce que la tâche 3 n'a jamais été
exécutée. Comparez avec le run précédent, où web2 finissait à `ok=2`.
**Le play entier est en échec** et rend un code de sortie non nul.
