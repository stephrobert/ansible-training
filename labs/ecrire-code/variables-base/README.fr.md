...
🔍 **Observation** : `from_vars_files` gagne, et c'est la **bonne surprise** de ce
lab. Dans la précédence Ansible, **`vars_files:` (niveau 14)** bat **`vars:` du
play (niveau 12)**. L'intuition dit le contraire : le `vars:` est écrit dans le
play, sous les yeux, donc il devrait primer. Mais le niveau seul décide, et le
fichier chargé est plus haut.

**Le piège fréquent est donc bien réel, simplement il va dans l'autre sens** : ce
n'est pas votre `vars_files:` qui est ignoré, c'est votre `vars:`. Quand une
valeur du play « ne prend pas », cherchez un `vars_files:` avant d'accuser
Ansible.
...
- **`vars:`** dans le play = portée play, prio moyenne (niveau 12).
- **`vars_files:`** = idem mais en fichier externe (niveau 14, juste **au-dessus** :
  le fichier **bat** le play, contre toute intuition).
...
3. Pourquoi `vars_files:` (niveau 14) bat-il `vars:` du play (niveau 12), alors qu'on
   pourrait imaginer le contraire (le `vars:` est écrit dans le play, donc "plus
   proche") ? Indice : lequel des deux porte une **décision d'exploitation**, et
   lequel porte un **défaut** ?
...
- **`include_vars:`** : charger des vars **dynamiquement** au runtime (selon une condition,
  selon un OS détecté). Différent de `vars_files:` qui est statique, et plus fort
  que lui (niveau 18 contre 14).
- **`set_fact:`** : créer une variable **côté managed node** pendant le play (niveau 19,
  bat le play vars comme le vars_files). Voir lab 16.
