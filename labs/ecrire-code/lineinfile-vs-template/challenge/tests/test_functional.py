...
def test_myapp_conf_server_workers(host):
    content = host.file("/etc/myapp.conf").content_string
    assert "workers = 4" in content, (
        "le template doit interpoler server.workers → 'workers = 4'"
    )


def test_myapp_conf_db_url(host):
    content = host.file("/etc/myapp.conf").content_string
    assert "url = postgres://db1.lab/myapp" in content, (
        "le template doit interpoler database.url → 'url = postgres://db1.lab/myapp'"
    )
