...
> Les mots-clés de **parallélisme** (`serial:`, `strategy:`, `max_fail_percentage:`) ne sont **pas** abordés ici — ils sont l'objet du [lab 09 — parallélisme et stratégies](../parallelisme-strategies/). Concentrez-vous d'abord sur l'**anatomie d'un play**.
...
Dans `pre_tasks:`, créez un fichier marqueur `/tmp/predeploy-{{ inventory_hostname }}.txt` via `ansible.builtin.copy` + `content:`. Indices :
...
🔍 **Observation à anticiper** : le contenu est **stable**, et c'est le point. Ce qui datera ce marqueur, c'est son `mtime`, que le noyau pose au moment de l'écriture. Vous le comparerez à celui du marqueur `postdeploy` à l'exercice 5.

> ⚠️ **Le réflexe à ne pas prendre** : écrire `content: "predeploy at {{ ansible_date_time.iso8601 }}"`. C'est tentant, et c'est faux deux fois.
>
> - **Ça ne date pas l'écriture.** `ansible_date_time` est un fact : il porte l'heure de la **collecte des facts**, pas celle de la tâche. Et `ansible.cfg` met les facts en cache pendant 2 heures (`fact_caching_timeout`) : d'un run à l'autre, et entre `pre_tasks` et `post_tasks`, ce fact est **gelé**. Vos deux marqueurs porteraient la même heure.
> - **Ça casse l'idempotence.** Un contenu qui change à chaque run fait rendre `changed` à `copy:` à chaque run. Le rôle a beau être « correct », il ment sur ce qu'il a fait.
>
> Un timestamp dans un `content:` est presque toujours le signe qu'on cherchait un `mtime`.
...
> Pour traiter **un hôte à la fois** (rolling update), il existe le mot-clé `serial:` — c'est l'objet du [lab 09](../parallelisme-strategies/).
...
🔍 **Observation** : le `mtime` du fichier `predeploy` doit être **strictement antérieur** au `mtime` du fichier `postdeploy`. C'est la preuve que `pre_tasks` s'exécute bien avant `post_tasks`, et remarquez d'où elle vient : de `ls -la`, pas du contenu des fichiers. Les deux marqueurs portent un texte identique à chaque run ; ce sont leurs dates de modification qui témoignent. C'est exactement ce que compare le test du challenge.
...
Le challenge ([`challenge/README.md`](challenge/README.md)) reproduit le pattern `pre_tasks` / `tasks` / `post_tasks` / `handlers` sur `db1.lab`, avec nginx comme dans le tutoriel : ce qui change, c'est l'hôte, pas le logiciel. Tests automatisés via `pytest+testinfra` :
...
- **`meta: flush_handlers`** : forcer le déclenchement immédiat des handlers en attente, sans attendre la fin de la section. Voir le [lab 06 — handlers](../handlers/).
...
- **Parallélisme et rolling updates** : `serial:`, `strategy:`, `max_fail_percentage:` sont introduits dans le [lab 09](../parallelisme-strategies/).
