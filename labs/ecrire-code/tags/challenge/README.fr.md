  ...
  tasks:
    - name: Marqueur always
      ansible.builtin.copy:
        dest: ???
        content: "always\n"          # contenu STABLE, cf. le piège ci-dessous
        mode: "0644"
      tags: ???
...
> - **Posez les marqueurs avec `copy:` et un contenu stable**, pas avec
>   `file: state: touch` ni `shell: touch`. Ces deux-là rendent `changed` à
>   **chaque** passage (touch écrit une nouvelle date de modification, c'est
>   sa raison d'être), et `test_solution_idempotente` exige `changed=0` au
>   second run. Si vous tenez à `touch`, il faut le neutraliser avec
>   `modification_time: preserve` et `access_time: preserve`.
> - **Pas de `{{ ansible_date_time.iso8601 }}` dans le `content:`** : le
>   contenu changerait à chaque run et casserait l'idempotence, pour ne rien
>   prouver de plus. Ce qui prouve qu'une tâche a tourné sous tel filtre de
>   tags, c'est que son marqueur **existe**. Le play tourne d'ailleurs avec
>   `gather_facts: false` : ce fact n'est même pas collecté.
