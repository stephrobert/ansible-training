...
> 💡 Le contenu est **stable**. Ce qu'un marqueur doit prouver ici, c'est que
> sa tâche a tourné sous tel filtre de tags : son **existence** le prouve. Y
> écrire `{{ ansible_date_time.iso8601 }}` ferait rendre `changed` à la tâche
> à chaque passage, et vous perdriez l'idempotence sans rien gagner. Si vous
> voulez savoir *quand* le marqueur a été posé, son `mtime` le dit déjà :
> `ls -l /tmp/tag-*.txt`.
...
🔍 **Observation** : `always` ignore le filtre `--tags`. **Cas d'usage typique** : vérification de prérequis, chargement de variables, pose d'un marqueur de run. À utiliser avec parcimonie : un play plein d'`always` est un play sans tags utiles.
