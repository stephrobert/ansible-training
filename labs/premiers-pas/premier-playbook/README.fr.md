...
AlmaLinux 9 a `firewalld` activé par défaut, donc le port 80 est **fermé** même si nginx écoute dessus. Ajoutez une tâche `ansible.posix.firewalld` qui ouvre le service `http` :
...
Le challenge ([`challenge/README.md`](challenge/README.md)) vous demande de **réécrire la même logique** pour `nginx` sur `db1.lab`, mais sur le port non standard **8888** au lieu du port 80 des webservers. Un simple changement de port fait apparaître un verrou de plus : SELinux. Les tests `pytest+testinfra` vérifieront automatiquement votre solution :
