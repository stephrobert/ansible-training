...
🔗 [**Mode pull avec ansible-pull (GitOps Edge)**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/pratiques/ansible-pull-gitops/)
...
- hosts: localhost
  connection: local
  become: true
  # true : le marqueur ci-dessous lit des facts. Avec gather_facts à false,
  # ansible_date_time et ansible_hostname resteraient indéfinies, et un
  # default() écrirait silencieusement « unknown »/« localhost » au lieu
  # d'échouer.
  gather_facts: true
  tasks:
