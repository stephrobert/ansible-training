...
Réponse attendue : 4 `pong` (un par managed node). Si KO, lancez `dsoxlab provision` à la racine.
...
ansible db1.lab -b -m ansible.builtin.dnf -a "name=tree state=absent"    # désinstaller un paquet
...
## 📚 Exercice 9 — Tout enchaîner

Enchaînez vous-même les 6 commandes de la boîte à outils, dans cet ordre :
