...
> mise install                 # installe Ansible + libvirt + outils (~3 min, 1×)
> dsoxlab provision                 # crée les 4 VMs + prépare les managed nodes (~5 min)
> mise run setup-hosts                 # ajoute web1.lab/web2.lab/db1.lab dans /etc/hosts (sudo)
> mise run setup-ssh            # 'ssh -F ~/.cache/dsoxlab/ansible-training/ssh_config web1.lab' utilise la clé du repo
> `dsoxlab status`                          # → 4 "pong" attendus (validation finale)
> ```
>
...
> | Voir l'état des hostnames du lab | `mise run setup-hosts` |
> | Snapshot avant un lab risqué | `mise run snapshot` |
> | Restaurer le snapshot | `mise run restore` |
> | Détruire toutes les VMs (après formation) | `dsoxlab destroy` |
> | Retirer les hostnames de `/etc/hosts` | `mise run remove-hosts` |
> | État de la config SSH du lab | `mise run setup-ssh` |
> | Retirer la config SSH du lab | `mise run remove-ssh` |
>
...
Réponse attendue : `web1.lab | SUCCESS => {"ping": "pong"}`. Si vous obtenez `UNREACHABLE`, lancez `dsoxlab provision` à la racine du repo.

> 💡 **Note** : ce lab est livré clé en main (`playbook.yml` + script Bash). Vous n'avez **rien à écrire** : vous allez **observer** la différence. Les labs suivants vous demanderont, eux, d'écrire le code.
...
Jouez le script **3 fois** d'affilée, et regardez ce qui s'accumule :
...
> 💡 **Détail technique** : `/usr/share/nginx/html/index.html` est en fait un **lien symbolique** vers `/usr/share/testpage/index.html`, un fichier livré par le paquet `almalinux-logos-httpd` (nginx le tire via `system-logos-httpd` : ce nom en `httpd` est un héritage historique de la page de test partagée, pas une dépendance à Apache). Le `tee -a` modifie donc le **vrai fichier** pointé par le lien, et c'est lui qui persiste entre les runs si rien ne le purge. Le `cleanup.yaml` du lab fait précisément ce nettoyage via `sed -i '/<p>Servi par /d' ...`.

## 📚 Exercice 3 — Voir le playbook Ansible converger

Réinitialisez l'état (`dsoxlab clean` désinstalle nginx et purge le fichier)
puis lancez **3 fois** le playbook :
...
- Le playbook **déclare** un état (`state: present`, `state: started`, ligne exacte via `lineinfile:` + `regexp:`). Le module `lineinfile:` **compare la ligne correspondant à la regexp** avant d'écrire : aucune écriture si elle dit déjà ce qu'il faut.
- Le playbook est **plus court** parce qu'il s'appuie sur l'idempotence native des modules `dnf`, `lineinfile`, `systemd`.
...
2. Le playbook utilise `lineinfile:` + `regexp:` pour poser une ligne d'`index.html`. Pourquoi est-ce idempotent même quand la ligne est déjà là ? Que compare le module ?
...
- **Remplacez `lineinfile:` par `copy:` + `content:`** dans `playbook.yml`. Comparez : `lineinfile:` est idempotent par regexp (une ligne, le reste du fichier est laissé tel quel), `copy:` par somme de contrôle (le fichier entier, qu'il écrase). Quand préférer l'un ou l'autre ?
