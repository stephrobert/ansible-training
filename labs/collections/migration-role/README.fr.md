...
│           ├── galaxy.yml
│           ├── meta/runtime.yml
│           ├── plugins/modules/lab97_check.py       ← renommé à la migration
...
- name: Status check via le module migré
  student.webapp.lab97_check:     # ← nouveau FQCN, nouveau nom
  register: status
...
🔍 **Observation** : à l'**intérieur** de la collection, on peut écrire des noms courts (`lab97_check:`), mais c'est **déconseillé**. Le FQCN explicite reste **lisible** et **portable**.
...
Au passage, on en profite pour **normaliser le nom** du module : `lab97_status`
dans le rôle standalone devient `lab97_check` dans la collection. C'est la
redirection qui fait survivre l'ancien nom.
...
plugin_routing:
  modules:
    lab97_status:                       # ← l'ANCIEN nom, celui qu'on fait survivre
      redirect: student.webapp.lab97_check   # ← le nom RÉEL dans la collection
      deprecation:
...
⚠️ **La clé et la cible doivent différer.** Écrire
`lab97_status: {redirect: student.webapp.lab97_status}` dans la collection
`student.webapp`, c'est rediriger `student.webapp.lab97_status` vers
lui-même. ansible-core refuse au runtime :
...
`plugin_routing` route un **nom** vers un **autre nom** : un renommage, ou un
déménagement vers une autre collection. Il ne « déplace » pas un module vers
l'endroit où il se trouve déjà. Un module simplement sorti du `library/` d'un
rôle **sans changer de nom** n'a besoin d'aucune route : le mot-clé
`collections:` du playbook, ou son FQCN, suffit à le résoudre.
...
- **La clé de la route et sa cible doivent différer** : rediriger un nom vers lui-même est une boucle, refusée au runtime.
