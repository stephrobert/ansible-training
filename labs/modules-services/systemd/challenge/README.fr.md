...
> 💡 **Pourquoi `chronyd` et pas `nginx` ?** `nginx` occupe déjà les ports
> 80/443 de `web1.lab` pour d'autres labs. `chronyd` (NTP, port 123 UDP) ne
> crée aucun conflit, et le module `systemd_service` s'utilise exactement de la
> même façon quel que soit le service : c'est lui le sujet, pas le démon.
...
>   `enabled: true`.
> - **Pas de `.service` dans `name:`** : juste `nginx`, pas
>   `nginx.service`. Sauf pour disambiguer (timer, socket).
