...
> ⚠️ Le module `archive` vit dans **`community.general`**, pas dans
> `ansible.builtin`. Attention au piège : `ansible.builtin.archive` **ne
> renvoie pas d'erreur**, le routing d'ansible-core redirige silencieusement
> (`ansible-doc ansible.builtin.archive` rend bien la page community.general).
> Votre playbook tourne, et `ansible-lint --profile production` le **rejette**
> quand même : règle `fqcn[canonical]`. Un module qui marche n'est pas un module
> correctement nommé.
>
> `unarchive`, lui, est bien dans `ansible.builtin`.
