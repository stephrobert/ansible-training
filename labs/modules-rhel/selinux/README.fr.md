...
🔗 [**Module SELinux Ansible**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/modules/systeme/module-selinux/)
...
de la politique. Ex : autoriser le serveur web à se connecter au réseau.

> 💡 Les booléens web sont préfixés `httpd_` par héritage historique, mais ils
> ne visent pas le paquet Apache : ils visent le **domaine SELinux `httpd_t`**,
> celui de tous les serveurs web de la politique targeted. nginx y tourne aussi
> (`ps -eZ | grep nginx` affiche `system_u:system_r:httpd_t:s0`), donc tout ce
> qui suit s'applique à la formation, qui déploie nginx.
...
| Booléen | Effet |
|---|---|
| `httpd_can_network_connect` | le serveur web peut faire des connexions sortantes |
| `httpd_can_network_connect_db` | le serveur web peut se connecter à une DB distante |
| `httpd_enable_homedirs` | le serveur web peut servir `~user/public_html/` |
...
Pattern fréquent : déployer une app web dans un dossier custom (pas la racine
web par défaut, `/usr/share/nginx/html/` sur RHEL). SELinux refuse au serveur
web de servir des fichiers qui n'ont pas le bon **contexte SELinux**.
...
1. Vous déployez une app dans `/opt/myapp/` qui doit être servie par nginx.
   nginx ne peut pas y accéder. Quel pipeline (booléen, sefcontext,
   restorecon) ?
...
- **`community.general.seport:`** : associer un **port** à un type SELinux.
  Le domaine `httpd_t` n'a le droit de se lier qu'à deux types : `http_port_t`
  (80, 81, 443, 488, 8008, 8009, 8443, 9000) et `http_cache_port_t` (8080,
  8118, 8123, 10001-10010). Tout autre port doit être étiqueté, sinon le bind
  échoue sur « Permission denied ». Attention au piège : **8080 marche déjà
  sans rien faire** (il est dans `http_cache_port_t`), donc ce n'est pas un bon
  exemple. Le lab `premiers-pas/premier-playbook` utilise 8888, qui lui est
  vraiment refusé par défaut. Vérifiez la politique de votre machine :
