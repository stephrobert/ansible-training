...
Les trois premières lignes divergent, les trois dernières non : c'est bien le
problème, **rien ne permet de le deviner**, il faut le savoir ou le vérifier.

```yaml
# ❌ Ne marche que sur RHEL : sur Debian, le paquet s'appelle « vim »
- ansible.builtin.package:
    name: vim-enhanced
    state: present
...
> 💡 L'exemple porte sur `vim-enhanced` parce que c'est un paquet que **ce lab
> installe vraiment** : vous pouvez vérifier son nom sur la VM. Le cas d'école
> le plus connu reste `httpd` (RHEL) contre `apache2` (Debian). À l'inverse,
> `nginx` (le serveur web de cette formation) porte le même nom partout : la
> divergence n'est pas une règle, c'est un piège ponctuel.
...
- **Noms de paquets diffèrent** entre distros (`vim-enhanced` vs `vim`, `httpd` vs `apache2`), mais pas toujours (`nginx` partout).
