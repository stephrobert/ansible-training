...
🔗 [**Module dnf Ansible**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/modules/paquets-services/module-dnf-options/)
...
🔍 **Observation** : quand on désinstalle `nginx`, ses dépendances exclusives
(`nginx-core`, `nginx-filesystem`, `almalinux-logos-httpd`) deviennent
**orphelines**. **`autoremove: true`** les supprime automatiquement, comme
`dnf autoremove` après un `dnf remove`.
