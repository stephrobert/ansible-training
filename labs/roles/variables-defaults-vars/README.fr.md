...
## 📚 Exercice 6 — Deux syntaxes presque identiques, deux résultats opposés

`vars/main.yml` du rôle « protège » ses constantes internes. Contre quoi, exactement ?
Faites le pari avant de lancer, puis mesurez. C'est le piège le plus subtil des rôles.

**Cas A : `vars:` au niveau du PLAY**
...
- name: Override au niveau du play
  ...
  become: true
  vars:
    __webserver_html_dir: /tmp/test    # ← au niveau du PLAY
  roles:
    - role: webserver
```

**Cas B : `vars:` sous l'entrée de rôle** (deux caractères d'indentation plus loin)
...
- name: Override sous l'entrée de rôle
  ...
  roles:
    - role: webserver
      vars:
        __webserver_html_dir: /tmp/test    # ← sous « - role: »
```

Lancez les deux, et regardez où atterrit la page d'accueil.

🔍 **Le résultat surprend, et c'est tout l'intérêt** :

| Cas | Ce que c'est vraiment | Précédence | Résultat |
| --- | --- | --- | --- |
| **A** : `vars:` du play | play vars | **12** | l'override est **IGNORÉ**, `vars/` du rôle (15) gagne |
| **B** : `vars:` sous `- role:` | **role params** (ce ne sont PAS des play vars) | **20** | l'override **S'APPLIQUE**, il bat `vars/` du rôle (15) |

**La leçon** : `vars/main.yml` ne protège **pas** un rôle « contre l'utilisateur ». Il le
protège contre un `vars:` posé au niveau du play, et **contre rien d'autre**. Qui passe la
variable en paramètre du rôle gagne, comme le fait `--extra-vars` (22) à l'exercice 7.

Le piège n'est pas la précédence, c'est la **syntaxe** : deux lignes d'écart dans le YAML,
et la variable change de catégorie. Un `vars:` sous `- role:` n'est pas un `vars:` de play.

> **Vérifiez-le vous-même**, ne me croyez pas : c'est exactement ce que fait
> `VariableManager.get_vars()` d'ansible-core, qui fusionne dans l'ordre `play vars`, puis
> `role vars`, puis `role params`. Le dernier fusionné écrase les précédents.
