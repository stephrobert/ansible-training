...
>   uniquement ce rôle, et c'est un **role param** (priorité 20), pas un
>   `vars:` de play (12). Deux lignes d'indentation d'écart, et la variable
>   change de catégorie : le role param bat même les `vars/main.yml` du rôle
>   (15), là où un `vars:` de play perd contre eux.
...
> - **Idempotence** : le handler `Notify deployment` est notifié par la tâche
>   « Tracer le déploiement », qui écrit un contenu **stable** (l'hôte et le
>   port appliqué). Elle ne rend donc `changed` que si l'état déployé change
>   vraiment. Au 2ème run identique, `changed=0` et aucun handler ne tourne :
>   c'est ce que vérifie `test_solution_idempotente`.
