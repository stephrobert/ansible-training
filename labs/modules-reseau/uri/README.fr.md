...
> 🔎 **L'API de ce lab tourne dans le lab.** `api-interne.lab:8009` est servie
> par nginx depuis web2.lab, montée par le `setup.yaml`. Aucun accès Internet
> n'est requis : ce lab tapait auparavant sur `httpbin.org`, qui rendait des
> 503 et des réponses à 26 secondes, et le faisait échouer pour une raison
> sans aucun rapport avec le module `uri`.
...
- Sans `status_code:`, le défaut est `[200]` — un 201 ferait failer la tâche.
  **Cette API répond justement 201** : retirez la ligne et regardez la tâche
  échouer. C'est le seul moyen de retenir la règle.
