...
Ce lab est joué **automatiquement** à la fin de `dsoxlab provision`. Il prépare les 3 managed nodes (`web1`, `web2`, `db1`) pour qu'ils soient utilisables par tous les playbooks de la formation.
...
ou via la CLI, qui pose l'état de départ puis valide :
...
Un second `dsoxlab run <id-du-lab>` doit afficher `changed=0` sur tous les nodes — c'est la définition d'un playbook propre. Si ce n'est pas le cas, c'est que le playbook réécrit quelque chose à chaque passage (typique : `lineinfile` avec une regexp non ancrée).

## Tests

Validation par `pytest + testinfra` dans [`challenge/tests/test_functional.py`](./challenge/tests/test_functional.py) — vérifie sur chaque managed node :
