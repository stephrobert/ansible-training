...
> Pré-requis : Molecule installé (cf. [lab 62](../introduction/)).

## 🧠 Rappel

🔗 [**Cycle TDD avec Molecule**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/roles/molecule-tdd-cycle/)
...
Vos tests sont rouges ? Écrivez maintenant le MINIMUM qui les fait
passer : une seule tâche `ansible.builtin.user` bouclant sur
`users_to_create`.
...
- name: Créer les utilisateurs
  ansible.builtin.user:
    name: "{{ item.name }}"
    shell: ???        # fallback sur users_default_shell (defaults du rôle)
    groups: ???       # groupes optionnels de l'item
    append: ???       # ne pas écraser les groupes existants
    state: present
  loop: ???
  loop_control:
    label: "{{ item.name }}"
...
🔍 **Observation** : un `loop:` sur la liste, `default(...)` pour les
champs optionnels, et le contrat (`argument_specs`, `defaults`) comme
