...
🔗 [**Tester un rôle Ansible avec Molecule**](https://blog.stephane-robert.info/docs/infra-as-code/gestion-de-configuration/ansible/roles/tdd-molecule-introduction/)
...
- **Conteneur ≠ VM** : Molecule + Podman teste très vite (~10-30 s) mais
  sans systemd réel par défaut. Pour tester systemd, il faut une image qui
  l'embarque : `pre_build_image: true` + une variante « init »
  (`docker.io/almalinux/10-init`) avec `command: /sbin/init` et
  `privileged: true`. Attention au piège : l'image de base d'une distro
  n'a PAS systemd (`docker.io/rockylinux/rockylinux:9` n'a même pas
  `/sbin/init`), et le rôle échoue alors sur le démarrage du service.
